...
class Tournament extends Mode {
  ...
  onTick(server) {
    ...
    if (server.run && this.roundStartTime) {
      var timePassed =
        Date.now() - (this.roundStartTime + this.accumulatedPauseTime);
      ...
      for (var i = 0, len = server.clients.length; i < len; i++) {
        ...
        server.clients[i].packetHandler.sendPacket(
          new Packet.UpdateTimer(timeLeft / 1000)
        );
      }
  ...
  updateLB(server, lb) {
    ...
    for (var i = 0, pos = 0; i < server.clients.length; i++) {
      ...
      if (
        ...
        (!server.config.minionsOnLeaderboard && player.isMi)
      )
        continue;
  ...
  roundStart(server) {
    ...
    for (var i = 0, len = server.clients.length; i < len; i++) {
        ...
        var Packet = require("../packet");
        //TODO: Optimize if needed (Only send every second)
        server.clients[i].packetHandler.sendPacketJSON(new Packet.StartCountdown(this.countdownTime))
      }

...
class Server {
  ...
  onServerSocketError(error) {
    ...
    switch (error.code) {
      case "EADDRINUSE":
        Logger.error(
          "Server could not bind to port " + this.config.serverPort + "!"
        );
        Logger.error(
  ...
  onClientSocketOpen(ws, req) {
    ...
    if (
      ...
      this.socketCount >= this.config.serverMaxConnections
    ) {
    ...
    if (
      this.config.clientBind.length &&
      req.headers.origin.indexOf(this.clientBind) < 0
    ) {
  ...
  checkMinion(ws, req) {
    ...
    if (this.config.serverMinionThreshold) {
      if (
        (ws.lastAliveTime - this.startTime) / 1000 >=
        this.config.serverMinionIgnoreTime
      ) {
  ...
  updateClients() {
    ...
    for (var i = 0; i < len; ) {
      ...
      if (
        this.clients[i].playerTracker.isRemoved ||
        this.clients[i].isCloseRequest
      )
        // remove dead client
    ...
    for (var i = 0, test = this.minionTest.length; i < test; ) {
      ...
      if (date > this.config.serverMinionInterval) this.minionTest.splice(i, 1);
      else i++;
    }
  }
  updateLeaderboard() {
  ...
  onChatMessage(from, to, message) {
    ...
    if (this.config.serverChatAscii) {
      for (var i = 0; i < message.length; i++) {
        if (
          (message.charCodeAt(i) < 0x20 || message.charCodeAt(i) > 0x7f) &&
          from
        ) {
          this.sendChatMessage(
            null,
            from,
            "Message failed - You can use ASCII text only!"
          );
          return;
        }
      }
    ...
    for (var i = 0, len = this.clients.length; i < len; i++) {
      ...
      if (!to || to == this.clients[i].playerTracker) {
        ...
        if (this.config.separateChatForTeams && this.mode.haveTeams) {
          ...
          if (
            from == null ||
            from.team === this.clients[i].playerTracker.team
          ) {
            this.clients[i].packetHandler.sendPacket(
              new Packet.ChatMessage(from, message)
            );
          }
        } else {
          this.clients[i].packetHandler.sendPacket(
  ...
  mainLoop() {
    ...
    if (!this.run && this.mode.IsTournament) this.ticks++;
    this.updateClients();
    // update leaderboard
    if ((this.ticks + 7) % 25 === 0) this.updateLeaderboard(); // once per second
    // ping server tracker
  ...
  updateSizeDecay(cell) {
    ...
    if (!rate || cell._size <= this.config.playerMinSize) return;
  ...
  autoSplit(cell, client) {
    ...
    if (client.rec) var maxSize = 1e9;
    // increase limit for rec (1 bil)
    else maxSize = this.config.playerMaxSize;
    // check size limit
    if (client.mergeOverride || cell._size < maxSize) return;
    if (
      client.cells.length >= this.config.playerMaxCells ||
      this.config.mobilePhysics
    ) {
      ...
      this.splitPlayerCell(client, cell, angle, cell._mass * 0.5);
    }
  }
  updateNodeQuad(node) {
    // update quad tree
  ...
  checkRigidCollision(m) {
    ...
    if (m.cell.owner != m.check.owner) {
      ...
      if (
        (this.mode.haveTeams && m.check.owner.isMi) ||
        (m.cell.owner.isMi && this.config.minionCollideTeam === 0)
      ) {
        return false;
      } else {
        // Different owners => same team
        return this.mode.haveTeams && m.cell.owner.team == m.check.owner.team;
      }
    }
  ...
  resolveCollision(m) {
    ...
    if (cell.owner && cell.owner == check.owner) {
      if (cell.getAge() < 13 || check.getAge() < 13) return; // just splited => ignore
  ...
  splitPlayerCell(client, parent, angle, mass) {
    ...
    // Too small to split
    if (!size1 || size1 < this.config.playerMinSize) return;
  ...
  spawnPlayer(player, pos) {
    ...
    if (this.willCollide(cell) && !player.isMi) pos = this.randomPos(); // Not safe => retry
    this.addNode(cell);
  ...
  splitCells(client) {
    ...
    cellToSplit.forEach((cell) => {
      ...
      if (cell._size < this.config.playerMinSplitSize) return; // cannot split
      // Get maximum cells for rec mode
      if (client.rec) var max = 200;
      // rec limit
      else max = this.config.playerMaxCells;
      if (client.cells.length >= max) return;
      // Now split player cells
      this.splitPlayerCell(client, cell, d.angle(), cell._mass * 0.5);
    });
  }
  canEjectMass(client) {
  ...
  ejectMass(client) {
    ...
    for (var i = 0; i < client.cells.length; i++) {
      ...
      if (newSize < 0 || newSize < minSize * minSize) continue; // Too small to eject
      ...
      var angle = d.angle() + Math.random() * 0.6 - 0.3;
      // Create cell and add it to node list
      var ejected;
      if (this.config.ejectVirus) {
        ejected = new Entity.Virus(this, null, pos, this.config.ejectSize);
      } else {
        ejected = new Entity.EjectedMass(
          this,
          null,
          pos,
          this.config.ejectSize
        );
      }
  ...
  loadFiles() {
    ...
    try {
      if (fs.existsSync(fileNameIpBan)) {
        ...
        this.ipBanList = fs
          .readFileSync(fileNameIpBan, "utf8")
          .split(/[\r\n]+/)
          .filter(function (x) {
    ...
    this.config.serverRestart =
      this.config.serverRestart === 0
        ? 1e999
        : this.config.serverRestart * 1500;
  }
  ...
  pingServerTracker() {
    ...
    var data =
      ...
      this.config.serverPort +
      "&gamemode=[**] " +
      this.mode.name + // we add [**] to indicate that this is MultiOgarII server
      ...
      os.arch() + // "win32 x64"
      "&uptime=" +
      process.uptime() + // Number of seconds server has been running
      "&version=MultiOgarII " +
      this.version +
      "&start_time=" +
      this.startTime;
...
function trackerRequest(options, type, body) {
  ...
  options.headers["content-length"] =
    body == null ? 0 : Buffer.byteLength(body, "utf8");
  var req = http.request(options, function (res) {
    if (res.statusCode != 200) {
      Logger.writeError(
        "[Tracker][" + options.host + "]: statusCode = " + res.statusCode
      );
      return;
    }
